...
public class Global
{
    ...
    {
        return StringUtils.nvl(getConfig("luaLu.static.domain"), "127.0.0.1");
    }

    /**
     * 获取设置的域名（ip地址），用来映射服务器静态资源路径
     */
    public static String getDomainPath()
    {
        String xxl = getConfig("luaLu.static.xxl").equalsIgnoreCase("true") ? "https://":"http://";
        String domain = getConfig("luaLu.static.domain");
        String domainPath = xxl + domain;
        return domainPath ;
    }
}
